...
class PerceptronClassifier:
        ...
        for i in range(self.max_iterations):
            ...
            for j in range(len(trainingData)):
                ...
                for k in self.legalLabels:
                        tempScore = sample * self.weights[k]
                ...
                if maxLabel != correctLabel:
                    self.weights[correctLabel] += sample
                    self.weights[maxLabel] -= sample
    ...
    def findHighWeightFeatures(self, label):
        ...
        #collect weights
        keys = self.weights[label].values()
        #sort weights to get greatest weights
        keys = sorted(keys,reverse=True)
        #filter top 100
        keys = keys [1:100]
        #identify unique weights among top 100
        keys = set(keys)
        """"
        loop through weights[label] and append the feature to 
        result if it's weight is one of the greatest weights
        """
